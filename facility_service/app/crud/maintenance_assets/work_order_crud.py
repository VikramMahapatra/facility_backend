...
def build_work_orders_filters(org_id: UUID, params: WorkOrderRequest):
    ...
    if params.search:
        ...
        filters.append(or_(WorkOrder.wo_no.ilike(search_term),
...
def get_work_orders(db: Session, org_id: UUID, params: WorkOrderRequest) -> WorkOrderListResponse:
    base_query = (
        db.query(WorkOrder, Asset.name.label('asset_name'), Space.name.label(
            'space_name'), Vendor.name.label('vendor_name'))
