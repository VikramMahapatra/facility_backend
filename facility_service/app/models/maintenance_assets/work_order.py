...
import uuid
from sqlalchemy import Boolean, Column, String, Text, ForeignKey, JSON, TIMESTAMP
...
class WorkOrder(Base):
    ...
    request_id = Column(UUID(as_uuid=True), ForeignKey(
        "service_requests.id", ondelete="SET NULL"), nullable=True)
    ...
    assigned_to = Column(UUID(as_uuid=True), ForeignKey(
        "vendors.id", ondelete="SET NULL"), nullable=True)
    ...
    service_requests = relationship(
        "ServiceRequest", back_populates="work_orders")
