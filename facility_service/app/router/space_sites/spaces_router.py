...
@router.post("/remove-owner", response_model=None)
def remove_owner(
    ...
    return crud.remove_space_owner(
        db=db,
        payload=payload
    )
...
@router.post("/remove-tenant", response_model=None)
def remove_space_tenant(
    ...
    return crud.remove_space_tenant(
        db=db,
        auth_db=auth_db,
        org_id=current_user.org_id,
        payload=payload
