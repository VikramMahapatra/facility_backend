...
from ..schemas.commercial_partners_schemas import CommercialPartnerOut, CommercialPartnerCreate, CommercialPartnerUpdate
from ..crud import commercial_partners_crud as crud

router = APIRouter(prefix="/api/commercial-partners",
                   tags=["commercial_partners"], dependencies=[Depends(validate_current_token)])
...
def read_partner(partner_id: str, db: Session = Depends(get_db)):
    ...
    if not db_partner:
        raise HTTPException(
            status_code=404, detail="CommercialPartner not found")
    return db_partner
