...
def create_user(db: Session, facility_db:Session, user : UserCreate):
    ...
    default_role = db.query(Roles).filter(func.lower(Roles.name) == user_role.lower()).first()
    ...
    token = auth.create_access_token({"user_id": str(user_instance.id), "org_id": str(user_instance.org_id),  "email": user_instance.email, "role": roles})
...
def get_user_by_id(facility_db:Session, user_data: Users):    
    user_org_data = facility_db.query(OrgSafe).filter(OrgSafe.id == user_data.org_id).first()
