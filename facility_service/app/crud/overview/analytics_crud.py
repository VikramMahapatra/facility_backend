...
def get_advance_analytics(db: Session, org_id: UUID, params: AnalyticsRequest):
    ...
    total_revenue = float(folio_charges + lease_charges +
                          invoice_totals) - float(abs(refunds) + cancellation_refunds)
    ...
    collection_rate = (float(total_collected) /
                       float(total_invoiced) * 100) if total_invoiced > 0 else 0
...
def get_revenue_analytics(db: Session, org_id: UUID, params: AnalyticsRequest):
    ...
    db_months = set([row.month for row in folio_months] +
                    [row.month for row in lease_months])
        ...
        utilities_lease = db.query(func.coalesce(func.sum(LeaseCharge.amount), 0))\
            ...
            .filter(
                *filters,
                LeaseCharge.period_start.between(month_start, month_end),
                LeaseCharge.amount > 0,
                LeaseChargeCode.code.in_(
                    ['ELEC', 'WATER', 'GAS', 'UTILITY'])
        ).scalar() or 0
        ...
        cam_revenue = db.query(func.coalesce(func.sum(LeaseCharge.amount), 0))\
            ...
            .filter(
                *filters,
                LeaseCharge.period_start.between(month_start, month_end),
                LeaseCharge.amount > 0,
                LeaseChargeCode.code == 'CAM'
        ).scalar() or 0

        # Categorize revenue
        rental_revenue = hotel_revenue + lease_revenue
        total_revenue = rental_revenue + cam_revenue + \
            parking_revenue + utilities_revenue
    ...
    if monthly_data:
        ...
        latest_data_month = max(
            [datetime.strptime(m['date'], '%Y-%m') for m in monthly_data])
        ...
        if len(months_with_revenue) >= 2:
            ...
            rental_growth = (last_month['rental'] - prev_month['rental']) / \
                prev_month['rental'] if prev_month['rental'] > 0 else 0.05
            cam_growth = (last_month['cam'] - prev_month['cam']) / \
                prev_month['cam'] if prev_month['cam'] > 0 else 0.05
            parking_growth = (last_month['parking'] - prev_month['parking']) / \
                prev_month['parking'] if prev_month['parking'] > 0 else 0.05
            utilities_growth = (last_month['utilities'] - prev_month['utilities']) / \
                prev_month['utilities'] if prev_month['utilities'] > 0 else 0.05

            # Generate forecast for next 3 months after latest data
            for i in range(1, 4):
                forecast_month = (latest_data_month +
                ...
                if forecast_month_str not in db_months:
                    forecast_rental = last_month['rental'] * \
                        (1 + rental_growth)
                    forecast_cam = last_month['cam'] * (1 + cam_growth)
                    forecast_parking = last_month['parking'] * \
                        (1 + parking_growth)
                    forecast_utilities = last_month['utilities'] * \
                        (1 + utilities_growth)
                    forecast_total = forecast_rental + forecast_cam + \
                        forecast_parking + forecast_utilities
...
def get_collection_performance(db: Session, org_id: UUID, params: AnalyticsRequest):
    ...
    start_date = (current_date.replace(day=1) -
                  timedelta(days=270))  # 9 months ago
    ...
    for month_str in all_months:
        ...
        total_invoiced = db.query(
            func.coalesce(func.sum(cast(func.jsonb_extract_path_text(
                Invoice.totals, 'grand'), Numeric())), 0)
        ...
        if total_invoiced_float > 0:
            collected_rate = min(
                100, max(0, (total_collected_float / total_invoiced_float) * 100))
            pending_rate = min(
                100, max(0, (total_pending / total_invoiced_float) * 100))
            overdue_rate = min(
                100, max(0, (overdue_amount_float / total_invoiced_float) * 100))
...
def get_site_profitability(db: Session, org_id: UUID, params: AnalyticsRequest):
    ...
    site_filters_from_params = [
        f for f in filters if hasattr(f, 'left') and f.left == Site.id]
    ...
    for site in sites:
        ...
        current_month_end = (
            current_month_start + timedelta(days=32)).replace(day=1) - timedelta(days=1)
        ...
        lease_revenue = db.query(func.coalesce(func.sum(LeaseCharge.amount), 0))\
            ...
            .filter(
                *site_filters,
                LeaseCharge.period_start.between(
                    current_month_start, current_month_end),
                LeaseCharge.amount > 0
        ).scalar() or Decimal('0')
        ...
        contract_costs = db.query(func.coalesce(func.sum(PurchaseOrderLine.qty * PurchaseOrderLine.price), 0))\
            ...
            .filter(
                *site_filters,
                ...
                PurchaseOrder.expected_date.between(
                    current_month_start, current_month_end)
        ).scalar() or Decimal('0')
        ...
        refund_expenses = db.query(func.coalesce(func.sum(BookingCancellation.refund_amount), 0))\
            ...
            .filter(
                *site_filters,
                BookingCancellation.cancelled_at.between(
                    current_month_start, current_month_end),
        ...
        if total_expenses == Decimal('0') and total_revenue > Decimal('0'):
            ...
            if site.kind == 'hotel':
                # Hotels typically have 50% margins
                expense_ratio = Decimal('0.5')
            elif site.kind == 'commercial':
                # Commercial properties have better margins
                expense_ratio = Decimal('0.4')
            elif site.kind == 'mall':
                # Malls have high revenue share models
                expense_ratio = Decimal('0.3')

            total_expenses = total_revenue * expense_ratio

        profit = total_revenue - total_expenses
        profit_margin = (profit / total_revenue) * \
            Decimal('100') if total_revenue > Decimal('0') else Decimal('0')
...
def get_occupancy_analytics(db: Session, org_id: UUID, params: AnalyticsRequest):
    ...
    start_date = current_date.replace(
        day=1) - timedelta(days=270)  # 9 months ago
    ...
    for month_str in all_months:
        ...
        occupancy_rate = (occupied_spaces / total_spaces) * \
            100 if total_spaces > 0 else 0
        available_rate = ((total_spaces - occupied_spaces - maintenance_spaces) /
                          total_spaces) * 100 if total_spaces > 0 else 0
        maintenance_rate = (maintenance_spaces / total_spaces) * \
            100 if total_spaces > 0 else 0
...
def get_space_type_performance(db: Session, org_id: UUID, params: AnalyticsRequest):
    ...
    for (space_kind,) in space_kinds:
        ...
        if space_kind == 'room':
            ...
            out_of_service = base_query.filter(
                Space.status == 'out_of_service').count()
            ...
            occupied = base_query.filter(Space.status.in_(
                ['occupied', 'checked_in'])).count()
...
def get_portfolio_distribution(db: Session, org_id: UUID, params: AnalyticsRequest):
    ...
    for (space_kind,) in space_kinds:
        ...
        occupancy_percentage = (
            occupied_count / total_count * 100) if total_count > 0 else 0
        ...
        distribution_data.append({
            ...
            # Occupancy percentage for this type
...
def get_yoy_performance(db: Session, org_id: UUID, params: AnalyticsRequest):
    ...
    current_occupancy = (current_occupied / current_total_spaces *
                         100) if current_total_spaces > 0 else 0
    ...
    previous_occupancy = (
        previous_occupied / current_total_spaces * 100) if current_total_spaces > 0 else 0
    ...
    occupancy_growth = current_occupancy - \
        previous_occupancy  # Absolute percentage point growth
    expenses_growth = ((current_expenses - previous_expenses) /
                       previous_expenses * 100) if previous_expenses > 0 else 0
    profit_growth = ((current_profit - previous_profit) /
                     previous_profit * 100) if previous_profit > 0 else 0
...
def get_site_comparison(db: Session, org_id: UUID, params: AnalyticsRequest):
    ...
    for site in sites:
        ...
        occupancy_rate = (total_occupied / total_spaces *
                          100) if total_spaces > 0 else 0
        ...
        satisfaction_query = db.query(func.avg(ServiceRequest.ratings))\
            .filter(
                ...
                # Only include rated requests
                ServiceRequest.ratings.isnot(None)
        )\
            .scalar() or 4.0

        satisfaction = round(satisfaction_query,
                             1) if satisfaction_query else 4.0
        ...
        efficiency = (completed_work_orders / total_work_orders *
                      100) if total_work_orders > 0 else 0
    ...
    site_comparison_data.sort(
        key=lambda x: x['metrics']['revenue'], reverse=True)
...
def get_maintenance_efficiency(db: Session, org_id: UUID, params: AnalyticsRequest):
    ...
    for total_stat in monthly_totals:
        ...
        completed_stat = next((c for c in monthly_completed if int(
            c.year) == year and int(c.month) == month), None)
        completed = completed_stat.completed if completed_stat else 0

        # Find matching pending count
        pending_stat = next((p for p in monthly_pending if int(
            p.year) == year and int(p.month) == month), None)
        pending = pending_stat.pending if pending_stat else 0

        # Find matching overdue count
        overdue_stat = next((o for o in monthly_overdue if int(
            o.year) == year and int(o.month) == month), None)
...
def get_energy_consumption(db: Session, org_id: UUID, params: AnalyticsRequest):
    ...
    utility_costs = db.query(
     ...
     .filter(
        ...
        extract('year', LeaseCharge.period_start) == current_year,
        LeaseChargeCode.code.in_(['ELEC', 'WATER', 'GAS']),
        *filters
    ).group_by(
    ...
    for (year,) in lease_years:
        ...
        for lease in leases_started:
            ...
            renewed_lease = db.query(Lease)\
                ...
                timedelta(days=90),  # Within 90 days of previous end
                    Lease.status == 'active',
                    *filters
            ).first()
        ...
        retention_rate = (renewals / total_leases *
                          100) if total_leases > 0 else 0
...
def get_daily_visitor_trends(db: Session, org_id: UUID, params: AnalyticsRequest):
    ...
    daily_stats = db.query(
        ...
        func.sum(case((AccessEvent.direction == 'in', 1), else_=0)
                 ).label('entries'),
...
def get_hourly_access_patterns(db: Session, org_id: UUID, params: AnalyticsRequest):
    ...
    for hour in range(24):
        ...
        hour_stats = next(
            (stat for stat in hourly_stats if int(stat.hour) == hour), None)
...
def get_portfolio_heatmap(db: Session, org_id: UUID, params: AnalyticsRequest):
    ...
    active_booking_space_ids = {
        booking.space_id for booking in active_bookings}
...
def get_performance_summary(db: Session, org_id: UUID, params: AnalyticsRequest):
    ...
    total_occupied = hotel_occupied + leased_occupied
    avg_occupancy = (total_occupied / total_spaces *
                     100) if total_spaces > 0 else 0
    ...
    collection_rate = (total_collected_float / total_invoiced_float *
                       100) if total_invoiced_float > 0 else 0.0
