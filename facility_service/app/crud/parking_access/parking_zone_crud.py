...
def update_parking_zone(db: Session, zone_update: ParkingZoneUpdate):  # ✅ Changed parameter
    ...
    db_zone = db.query(ParkingZone).filter(ParkingZone.id == zone_update.id,
                                           # ✅ Get ID from zone_update
                                           ParkingZone.is_deleted == False).first()
    if not db_zone:
        return error_response(
    ...
    result = (
        ...
        .join(Site, ParkingZone.site_id == Site.id)
        # ✅ Use zone_update.id
        .filter(ParkingZone.id == zone_update.id, ParkingZone.is_deleted == False)
        .first()
    )
...
def delete_parking_zone(db: Session, zone_id: str):
    ...
    db_zone = db.query(ParkingZone).filter(
        ParkingZone.id == zone_id, ParkingZone.is_deleted == False).first()
    if not db_zone:
        return None
